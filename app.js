...
const categoryLabel=v=>({hotel:"住宿",flight:"航班",train:"火車",restaurant:"餐廳",activity:"活動",ticket:"票券",rental_car:"租車",other:"其他"}[v]||v||"");
...
function reservationBadge(reservation){
  ...
  return `<div class="reservation-line"><span class="reservation-label">預訂狀態</span><span class="badge ${esc(reservation.status||"")}">${esc(statusLabel(reservation.status))}</span></div>`;
}
...
async function renderToday(force=false){
  ...
  const [items,reservations]=await Promise.all([api("itinerary",{date:state.date,group:groupParam()},force),api("reservations",{group:groupParam()},force)]);
  const reservationById=new Map(reservations.map(r=>[r.id,r]));
  app.innerHTML=`<section class="section">${dateNav()}${filters()}</section><section class="section"><h2>今日行程</h2><div class="card">${items.length?items.map(i=>{const r=i.reservation_id?reservationById.get(i.reservation_id):null;return `<div class="timeline-item"><div class="time">${esc(i.start_time||"—")}</div><div class="timeline-body"><div class="row"><h3>${esc(i.title)}</h3><span class="badge ${esc(i.certainty)}">${esc(statusLabel(i.certainty))}</span></div>${i.description?`<div class="meta">${esc(i.description)}</div>`:""}<div class="meta">${esc(i.city||"")}${i.group?` · ${esc(groupLabel(i.group))}`:""}</div>${reservationBadge(r)}</div></div>`}).join(""):'<div class="empty">這一天沒有行程。</div>'}</div></section>`;
...
function renderMap(){pageTitle.textContent="地圖";app.innerHTML='<div class="card empty"><div class="placeholder-icon">⌖</div><strong>地圖功能準備中</strong><div class="meta">下一步會使用地點資料中的 Google Maps 連結，直接開啟外部地圖，不先引入 Maps SDK。</div></div>'}
function renderMore(){pageTitle.textContent="更多";app.innerHTML=`<div class="stack"><div class="card"><h2>旅遊行程規劃</h2><div class="meta">旅客唯讀版 · API v1 · P3.1</div></div><div class="card"><h3>目前使用的 API</h3><div class="meta" style="word-break:break-all">${esc(config.apiBase)}</div></div></div>`}
