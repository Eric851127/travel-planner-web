...
function doGet(e) {
  ...
  if (mode === 'probe') return p9Json_({ok:true,phase:'P9.3-session',transport:'GET',at:new Date().toISOString()});
  if (mode === 'config') return p9PublicConfig_();
  if (mode === 'auth_start') return p9AuthStart_(e);
  return p9Json_({ok:true,phase:'P9.3-session',message:'Isolated authentication/session API. No Admin CRUD is exposed.'});
}

function doPost(e) {
  const mode = String((e && e.parameter && e.parameter.mode) || '').trim();
  if (mode === 'probe') return p9Json_({ok:true,phase:'P9.3-session',transport:'POST',echo:String((e && e.parameter && e.parameter.echo)||'').slice(0,120),at:new Date().toISOString()});
  if (mode === 'auth_finish') return p9AuthFinish_(e);
  if (mode === 'session_check') return p9SessionCheck_(e);
  if (mode === 'session_logout') return p9Json_({ok:true,logged_out:true});
...
function p9AuthFinish_(e) {
  try {
    ...
    // Compatibility path kept from the proven P9.2 flow.
...
function p9SessionCheck_(e) {
  try {
    ...
    return p9Json_({
      ...
      active:false,
      admin:false,
      error:p9Error_(error,'SESSION_INVALID')
    });
  }
}
...
function p9IssueSession_(identity) {
  ...
  const payload = {
    ...
    email:email,
    iat:now,
    exp:now + P9_CONFIG.SESSION_TTL_SECONDS,
    nonce:p9RandomToken_()
  };
...
function p9VerifySessionToken_(token) {
  ...
  let payload;
  try {
    payload = JSON.parse(p9Base64UrlDecodeUtf8_(parts[1]));
  } catch (_) {
...
function p9SessionSecret_() {
  const secret = String(
    PropertiesService.getScriptProperties().getProperty('P9_SESSION_SECRET') || ''
  ).trim();
...
function p9ConsumeState_(state) {
  ...
  if (
    !stateData ||
    !String(stateData.returnUrl || '').trim() ||
    !String(stateData.codeVerifier || '').trim() ||
    !Number(stateData.createdAt)
  ) throw new Error('INVALID_OR_EXPIRED_STATE');

  if (Date.now() - Number(stateData.createdAt) > P9_CONFIG.STATE_TTL_SECONDS * 1000) {
    throw new Error('INVALID_OR_EXPIRED_STATE');
  }
  return stateData;
}
...
function p9PkceVerifier_() {
  return (p9RandomToken_() + p9RandomToken_()).slice(0,96);
}
...
function p9RunSecuritySelfTests() {
  ...
  record('return_url exact allowlist', function(){
    if (p9ValidateReturnUrl_(props.returnUrl,props.returnUrl) !== props.returnUrl) throw new Error('ALLOWLIST_MISMATCH');
  });

  record('return_url rejects foreign URL', function(){
    expectError('RETURN_URL_NOT_ALLOWED',function(){p9ValidateReturnUrl_('https://example.com/evil',props.returnUrl);});
  });
  ...
  record('state TTL <= 600 seconds', function(){
  ...
  record('state single-use replay rejected', function(){
    ...
    CacheService.getScriptCache().put(
      'p9state:'+state,
      JSON.stringify({returnUrl:props.returnUrl,codeVerifier:p9PkceVerifier_(),createdAt:Date.now()}),
      60
    );
  ...
  record('wrong audience rejected',function(){
  ...
  record('expired token rejected',function(){
    const c=Object.assign({},goodClaims,{exp:now-5});
  ...
  record('signed session accepted',function(){
    const session=p9IssueSession_({sub:'self-test-sub',email:'self-test@example.com'});
    const identity=p9VerifySessionToken_(session.token);
    if (identity.sub !== 'self-test-sub' || identity.email !== 'self-test@example.com') {
      throw new Error('SESSION_IDENTITY_MISMATCH');
    }
  });

  record('tampered session rejected',function(){
    const session=p9IssueSession_({sub:'self-test-sub',email:'self-test@example.com'});
    const parts=session.token.split('.');
    parts[1]=parts[1].slice(0,-1)+(parts[1].slice(-1)==='A'?'B':'A');
    expectError('SESSION_INVALID',function(){p9VerifySessionToken_(parts.join('.'));});
  });

  record('expired session rejected',function(){
    const payload={
      v:1,sub:'self-test-sub',email:'self-test@example.com',
      iat:now-P9_CONFIG.SESSION_TTL_SECONDS-10,
      exp:now-1,nonce:p9RandomToken_()
    };
    const encoded=p9Base64UrlEncodeUtf8_(JSON.stringify(payload));
    const input='tp1.'+encoded;
    const token=input+'.'+p9SessionSignature_(input);
    expectError('SESSION_EXPIRED',function(){p9VerifySessionToken_(token);});
  });

  record('future session rejected',function(){
    const payload={
      v:1,sub:'self-test-sub',email:'self-test@example.com',
      iat:now+120,exp:now+300,nonce:p9RandomToken_()
    };
    const encoded=p9Base64UrlEncodeUtf8_(JSON.stringify(payload));
    const input='tp1.'+encoded;
    const token=input+'.'+p9SessionSignature_(input);
    expectError('SESSION_NOT_YET_VALID',function(){p9VerifySessionToken_(token);});
  });

  const passed=tests.filter(function(t){return t.pass;}).length;
  const result={phase:'P9.3-session',ok:passed===tests.length,passed:passed,total:tests.length,tests:tests};
  console.log(JSON.stringify(result,null,2));
  return result;
}
...
function p9Error_(error,fallback) {
  return String((error && error.message)||error||fallback||'INTERNAL_ERROR').trim();
}
