...
function p10RunCrudGateAuthorized_(admin){
  ...
  try{
    ids.placeA=run('create place A',function(){return p10SaveEntityAuthorized_('places',{name:marker+' A',city:'Tokyo',category:'other',address:'',opening_hours:'',website:'',notes:'temporary P10 gate'},'P',p10ValidatePlace_);}).id;
    ids.placeB=run('create place B',function(){return p10SaveEntityAuthorized_('places',{name:marker+' B',city:'Tokyo',category:'other',address:'',opening_hours:'',website:'',notes:'temporary P10 gate'},'P',p10ValidatePlace_);}).id;
    ...
    run('update place',function(){return p10SaveEntityAuthorized_('places',{id:ids.placeA,name:marker+' A',city:'Tokyo',category:'attraction',address:'',opening_hours:'',website:'',notes:'updated'},'P',p10ValidatePlace_);});
    ...
    const checks={
      ...
      place:bootstrap.places.some(function(r){return r.id===ids.placeA&&r.category==='attraction';})
    };
...
function p10RunSelfTests(){
  ...
  record('place category enum matches Sheet validation',function(){const expected=['station','airport','hotel','restaurant','attraction','shop','other'];if(JSON.stringify(P10_CONFIG.PLACE_CATEGORIES)!==JSON.stringify(expected))throw new Error('PLACE_CATEGORY_ENUM_MISMATCH');});
