...
function saveEntity_(sheetKey,input,prefix,validator){assertAdminAuthorized_();return withWriteLock_(function(){const ss=SpreadsheetApp.openById(ADMIN_CONFIG.SPREADSHEET_ID),sheet=requiredSheet_(ss,ADMIN_CONFIG.SHEETS[sheetKey]),table=readTable_(sheet),clean=validator(input||{},table.objects,ss),existingIndex=clean.id?table.objects.findIndex(function(r){return r.id===clean.id;}):-1;if(existingIndex>=0){const merged=Object.assign({},table.objects[existingIndex],clean);writeObjectRow_(sheet,existingIndex+2,table.headers,merged);return{success:true,action:'updated',id:merged.id,location:sheetKey==='places'?placeLocationResult_(merged):null};}const id=nextId_(table.objects.map(function(r){return r.id;}),prefix,3),created=Object.assign({},clean,{id:id});if(sheetKey==='itinerary'){created.day=computeDay_(clean.date,table.objects);created.order=nextItineraryOrder_(clean.date,table.objects);}appendObjectRow_(sheet,table.headers,created);return{success:true,action:'created',id:id,location:sheetKey==='places'?placeLocationResult_(created):null};});}
function placeLocationResult_(row){return{located:hasCoordinatePair_(row.latitude,row.longitude),latitude:row.latitude||'',longitude:row.longitude||'',address:row.address||'',google_maps_url:row.google_maps_url||''};}
function appendObjectRow_(sheet,headers,obj){const lastRow=sheet.getLastRow(),newRow=lastRow+1;if(newRow>sheet.getMaxRows())sheet.insertRowsAfter(sheet.getMaxRows(),1);if(lastRow>=2){const source=sheet.getRange(lastRow,1,1,headers.length),target=sheet.getRange(newRow,1,1,headers.length);source.copyTo(target,SpreadsheetApp.CopyPasteType.PASTE_FORMAT,false);source.copyTo(target,SpreadsheetApp.CopyPasteType.PASTE_DATA_VALIDATION,false);}sheet.getRange(newRow,1,1,headers.length).setValues([headers.map(function(h){return valueForSheet_(obj[h]);})]);}
...
function validatePlace_(input,rows,ss){
  const o=validateExistingId_(input.id,rows,/^P\d{3,}$/,'地點');
  o.name=requiredText_(input.name,'地點名稱',160);o.city=text_(input.city,100);o.category=text_(input.category,80);o.address=text_(input.address,300);o.opening_hours=text_(input.opening_hours,500);o.website=optionalHttpsUrl_(input.website,'website');o.notes=text_(input.notes,1000);
  const existing=input.id?rows.find(function(r){return r.id===input.id;}):null;
  const sameLocation=!!existing&&String(existing.name||'').trim()===o.name&&String(existing.city||'').trim()===o.city&&String(existing.address||'').trim()===o.address;
  if(sameLocation&&hasCoordinatePair_(existing.latitude,existing.longitude)){
    o.latitude=Number(existing.latitude);o.longitude=Number(existing.longitude);o.google_maps_url=String(existing.google_maps_url||mapsUrlForCoordinates_(o.latitude,o.longitude));
    return o;
  }
  const located=geocodePlace_(o.name,o.city,o.address);
  if(located){o.latitude=located.latitude;o.longitude=located.longitude;if(!o.address&&located.formatted_address)o.address=text_(located.formatted_address,300);o.google_maps_url=mapsUrlForCoordinates_(o.latitude,o.longitude);}
  else {o.latitude='';o.longitude='';o.google_maps_url='';}
  return o;
}
function geocodePlace_(name,city,address){
  const query=String(address||'').trim()||[String(name||'').trim(),String(city||'').trim(),'Japan'].filter(Boolean).join(', ');
  if(!query)return null;
  try{
    const response=Maps.newGeocoder().setRegion('jp').setLanguage('ja').geocode(query);
    if(!response||!response.results||!response.results.length)return null;
    const result=response.results[0],loc=result&&result.geometry&&result.geometry.location;
    if(!loc||!Number.isFinite(Number(loc.lat))||!Number.isFinite(Number(loc.lng)))return null;
    return{latitude:Number(loc.lat),longitude:Number(loc.lng),formatted_address:result.formatted_address||''};
  }catch(err){console.warn('Place geocode failed: '+query+' / '+err);return null;}
}
function mapsUrlForCoordinates_(lat,lng){return'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(String(lat)+','+String(lng));}
function hasCoordinatePair_(lat,lng){return lat!==null&&lat!==undefined&&String(lat).trim()!==''&&lng!==null&&lng!==undefined&&String(lng).trim()!==''&&Number.isFinite(Number(lat))&&Number.isFinite(Number(lng));}
